"""
MMBert Modeling Pipeline
Refactored from MMBert_Phase_3_4.ipynb
"""
...
def train_bilstm(X, y, cv_splits, model_name="MMBert", output_dir="MMBert_Models",
    ...
    for fold, (train_idx, test_idx) in enumerate(cv_splits, 1):
        ...
        torch.save(model, f"{output_dir}/BiLSTM/MMBert_Fold{fold}.pt")
        torch.save(model.state_dict(), f"{output_dir}/BiLSTM/MMBert_Fold{fold}.pth")
    ...
    res_df.to_csv(f"{output_dir}/BiLSTM/MMBert_bilstm_fold_results.csv", index=False)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{output_dir}/BiLSTM/MMBert_bilstm_accuracy_plot.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/BiLSTM/MMBert_bilstm_confusion_matrix.png")
    plt.close()
...
def main():
    """Main execution function for MMBert modeling."""
    print("=" * 70)
    print("MMBERT MODELING PIPELINE")
    print("=" * 70)
    
    # Load data
    print("Loading data...")
    with open("x_mmbert_1.pkl", "rb") as f:
        X = pickle.load(f)
    ...
    try:
        with open("new_cv_folds.pkl", "rb") as f:
            cv_splits = pickle.load(f)
        print("Loaded existing CV splits")
    except:
        cv_splits = create_cv_splits(X, y, groups, output_file="new_cv_folds.pkl")
    
    # Train models
    train_logistic_regression(X, y, cv_splits, "MMBert", "MMBert_Models")
    train_random_forest(X, y, cv_splits, "MMBert", "MMBert_Models")
    train_xgboost(X, y, cv_splits, "MMBert", "MMBert_Models")
    
    # Load sequence data for BiLSTM
    with open("x_mmbert_2.pkl", "rb") as f:
        X_seq = pickle.load(f)
    
    train_bilstm(X_seq, y, cv_splits, "MMBert", "MMBert_Models")
    
    print("\nMMBert modeling complete!")
