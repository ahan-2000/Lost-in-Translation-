"""
ByT5 Modeling Pipeline
Refactored from ByT5_Phase_3_4.ipynb
"""
...
def train_bilstm(X, y, cv_splits, model_name="ByT5", output_dir="ByT5_Models",
    ...
    for fold, (train_idx, test_idx) in enumerate(cv_splits, 1):
        ...
        torch.save(model, f"{output_dir}/BiLSTM/ByT5_Fold{fold}.pt")
        torch.save(model.state_dict(), f"{output_dir}/BiLSTM/ByT5_Fold{fold}.pth")
    ...
    res_df.to_csv(f"{output_dir}/BiLSTM/byt5_bilstm_fold_results.csv", index=False)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{output_dir}/BiLSTM/byt5_bilstm_accuracy_plot.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/BiLSTM/byt5_bilstm_confusion_matrix.png")
    plt.close()
...
def main():
    """Main execution function for ByT5 modeling."""
    print("=" * 70)
    print("BYT5 MODELING PIPELINE")
    print("=" * 70)
    
    # Load data
    print("Loading data...")
    with open("x_byt5_1.pkl", "rb") as f:
        X = pickle.load(f)
    ...
    # Train models
    train_logistic_regression(X, y, cv_splits, "ByT5", "ByT5_Models")
    train_random_forest(X, y, cv_splits, "ByT5", "ByT5_Models")
    train_xgboost(X, y, cv_splits, "ByT5", "ByT5_Models")
    
    # Load sequence data for BiLSTM
    with open("x_byt5_2.pkl", "rb") as f:
        X_seq = pickle.load(f)
    
    train_bilstm(X_seq, y, cv_splits, "ByT5", "ByT5_Models")
    
    print("\nByT5 modeling complete!")
