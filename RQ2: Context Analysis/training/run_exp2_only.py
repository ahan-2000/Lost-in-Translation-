#!/usr/bin/env python3
"""
Exp2 Only Training Script with Attention Visualization
Runs only the ContextReaderModel (Exp2) and saves attention visualizations
"""
...
from word_context_analysis.training.models import ContextReaderModel
...
def parse_args():
    ...
    ap.add_argument("--hidden", type=int, default=256)  # Revert to original
    ap.add_argument("--dropout", type=float, default=0.2)  # Revert to original
...
def visualize_attention(model, loader, tokenizer, device, save_dir, fold, num_samples=10):
    ...
    for i, data in enumerate(attention_data):
        ...
        fig.suptitle(f'Fold {fold} - Sample {i+1}: {"✓" if data["true_label"] == data["pred_label"] else "✗"}', 
                     fontsize=16)
...
def main():
    ...
    outdir = os.path.join("outputs", f"exp2_only_{stamp}")
    ...
    for fold, (tr_idx, va_idx) in enumerate(splits, start=1):
        ...
        # Train
        rprint(f"[bold green]Training Exp2 (ContextReader) - Fold {fold}[/]")
        exp2 = train_model(exp2, tr_loader, va_loader, 
                          lr=args.lr, weight_decay=args.weight_decay,  # Revert to original LR
                          epochs=args.epochs, warmup_ratio=args.warmup_ratio, device=device,
                          experiment_name="Exp2_ContextReader", fold=fold, 
                          class_weights=class_weights, save_dir=outdir, patience=3)  # Reduce patience
    ...
    results_df.to_csv(os.path.join(outdir, "exp2_results.csv"), index=False)
    ...
    with open(os.path.join(outdir, "exp2_summary.json"), "w") as f:
        json.dump(summary, f, indent=2)
    
    rprint(f"[bold green]Exp2 Training Complete![/]")
